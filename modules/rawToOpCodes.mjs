...
async function fetchData() {
    ...
    try {
        ...
        CHAPTER_DB = data['story']
        MUSIC = data['music']
        BACKGROUNDS = data['bg']
    } catch {
        let data = await (await fetch('https://gfl.amaryllisworks.pw/chapterDatabase.json')).json()

        PORTRAITS = await (await fetch('https://gfl.amaryllisworks.pw/portraitInformation.json')).json()
        CHAPTER_DB = data['story']
        MUSIC = data['music']
        BACKGROUNDS = data['bg']
...
/**
 * This function takes a string of gfl script, then returns structured lines. It produces no side effects.
 * @author AmWorks
 * @date 2021-11-21
 * @param {string} out
 ...
 * @returns {any[]}
 */
async function convertGFLTextToOpcodes(out, missionID, fileName = "") {
    if (MUSIC === null) await fetchData();
...
export { OPCODES, convertGFLTextToOpcodes }